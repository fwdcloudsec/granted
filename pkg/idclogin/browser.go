...
func openBrowser(url string, browserProfile string) error {
	...
	if config.SSOBrowserLaunchTemplate != nil {
		return openWithLaunchTemplate(config, url, browserProfile)
	}
...
func OpenBrowserWithFallbackMessage(url string, browserProfile string) error {
	clio.Info("If the browser does not open automatically, please open this link: " + url)
	err := openBrowser(url, browserProfile)
	if err != nil {
